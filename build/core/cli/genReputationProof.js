...
const genReputationProof = async (args) => {
    ...
    if (repNullifiersAmount > 0) {
        ...
        for (let n = 0; n < Number(rep.posRep) - Number(rep.negRep); n++) {
            const reputationNullifier = (0, src_1.genReputationNullifier)(id.getNullifier(), epoch, n, attesterId);
        ...
        if (nonceStarter + repNullifiersAmount >
    ...
    for (let i = repNullifiersAmount; i < src_1.maxReputationBudget; i++) {
        nonceList.push(BigInt(-1));
    }
    const graffitiPreImage = args.graffiti_preimage != null
