...
const deploy = async (args) => {
    ...
    // Epoch length
    const _epochLength = args.epoch_length != undefined
        ? args.epoch_length
        : defaults_1.DEFAULT_EPOCH_LENGTH;
    // Attesting fee
    const _attestingFee = args.attesting_fee != undefined
        ? ethers_1.ethers.BigNumber.from(args.attesting_fee)
        : defaults_1.DEFAULT_ATTESTING_FEE;
    const settings = {
    ...
    if (!(await (0, utils_1.checkDeployerProviderConnection)(deployerPrivkey, provider))) {
        console.error('Error: unable to connect to the Ethereum provider at', ethProvider);
        return;
    }
    const deployer = (0, utils_1.genJsonRpcDeployer)(deployerPrivkey, provider);
    debugger;
