...
class UserState {
    constructor(_unirepState, _id, _hasSignedUp, _latestTransitionedEpoch, _latestGSTLeafIndex, _latestUserStateLeaves, _transitionedFromAttestations) {
         ...
         */
        this.signUp = async (epoch, identityCommitment, attesterId, airdropAmount, blockNumber) => {
            // update unirep state
            await this.unirepState.signUp(epoch, identityCommitment, attesterId, airdropAmount, blockNumber);
            ...
            if (identityCommitment === this.commitment) {
                ...
                if (attesterId && airdropAmount) {
                    const stateLeave = {
                        ...
                        reputation: Reputation.default().update(BigInt(airdropAmount), BigInt(0), BigInt(0), BigInt(signUpInLeaf)),
                    };
        ...
        this.userStateTransition = async (fromEpoch, GSTLeaf, nullifiers, blockNumber) => {
            if (this.hasSignedUp && this.latestTransitionedEpoch === fromEpoch) {
                ...
                if (epkNullifiersMatched == this.numEpochKeyNoncePerEpoch) {
                    ...
                    console.error(`Number of epoch key nullifiers matched ${epkNullifiersMatched} not equal to numEpochKeyNoncePerEpoch ${config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH}`);
                    return;
                }
            }
        ...
        this.genUserStateTransitionProofs = async () => {
            ...
            return {
                ...
                finalTransitionProof: {
                    ...
                    epochKeyNullifiers: finalProofResults['publicSignals'].slice(1, 1 + config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH),
                    transitionedFromEpoch: finalProofResults['publicSignals'][1 + config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH],
                    blindedUserStates: finalProofResults['publicSignals'].slice(2 + config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH, 4 + config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH),
                    fromGSTRoot: finalProofResults['publicSignals'][4 + config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH],
                    blindedHashChains: finalProofResults['publicSignals'].slice(5 + config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH, 5 + 2 * config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH),
                    fromEpochTree: finalProofResults['publicSignals'][5 + 2 * config_1.NUM_EPOCH_KEY_NONCE_PER_EPOCH],
                },
            };
        };
        /*
        ...
        this.genProveReputationProof = async (attesterId, epkNonce, minRep, proveGraffiti, graffitiPreImage, nonceList) => {
            ...
            return {
                proof: results['proof'],
                publicSignals: results['publicSignals'],
                reputationNullifiers: results['publicSignals'].slice(0, config_1.MAX_REPUTATION_BUDGET),
                epoch: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET],
                epochKey: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 1],
                globalStatetreeRoot: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 2],
                attesterId: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 3],
                proveReputationAmount: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 4],
                minRep: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 5],
                proveGraffiti: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 6],
                graffitiPreImage: results['publicSignals'][config_1.MAX_REPUTATION_BUDGET + 7],
            };
        };
        ...
        this.numAttestationsPerProof = config_1.NUM_ATTESTATIONS_PER_PROOF;
        this.id = _id;
