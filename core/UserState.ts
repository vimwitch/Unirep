...
import { Circuit, genProofAndPublicSignals } from '../circuits'
import {
...
class UserState {
    ...
    public userStateTransition = async (
        ...
        if (this.hasSignedUp && this.latestTransitionedEpoch === fromEpoch) {
            ...
            if (epkNullifiersMatched == this.numEpochKeyNoncePerEpoch) {
                ...
                console.error(
                    `Number of epoch key nullifiers matched ${epkNullifiersMatched} not equal to numEpochKeyNoncePerEpoch ${NUM_EPOCH_KEY_NONCE_PER_EPOCH}`
                )
                return
            }
        }
        ...
        return {
            ...
            finalTransitionProof: {
                ...
                epochKeyNullifiers: finalProofResults['publicSignals'].slice(
                    1,
                    1 + NUM_EPOCH_KEY_NONCE_PER_EPOCH
                ),
                ...
                blindedUserStates: finalProofResults['publicSignals'].slice(
                    2 + NUM_EPOCH_KEY_NONCE_PER_EPOCH,
                    4 + NUM_EPOCH_KEY_NONCE_PER_EPOCH
                ),
                fromGSTRoot:
                    finalProofResults['publicSignals'][
                        4 + NUM_EPOCH_KEY_NONCE_PER_EPOCH
    ...
    public genProveReputationProof = async (
        ...
        return {
            ...
            epoch: results['publicSignals'][MAX_REPUTATION_BUDGET],
            epochKey: results['publicSignals'][MAX_REPUTATION_BUDGET + 1],
            globalStatetreeRoot:
                results['publicSignals'][MAX_REPUTATION_BUDGET + 2],
            attesterId: results['publicSignals'][MAX_REPUTATION_BUDGET + 3],
            proveReputationAmount:
                results['publicSignals'][MAX_REPUTATION_BUDGET + 4],
            minRep: results['publicSignals'][MAX_REPUTATION_BUDGET + 5],
            proveGraffiti: results['publicSignals'][MAX_REPUTATION_BUDGET + 6],
            graffitiPreImage:
                results['publicSignals'][MAX_REPUTATION_BUDGET + 7],
        }
    }
